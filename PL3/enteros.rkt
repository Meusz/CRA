...
;;;;IMPLEMENTACIONES ADICIONALES PL3´

;;;Cabeza de una lista
(define cabeza (lambda(x)
                            (primero (segundo x)))
  )

;;;Cola de una lista
(define cola (lambda(x)
                     (segundo (segundo x)))
  )
;;;Definición de lista vacia según apuntes (pág. 60):
...
(define vacia? (lambda (lst)
                 (primero lst) )
  )

;Lista mide 1
(define igualar (lambda (x)
                  (lambda (y)
                    (= x y) ))
  )
(define len_1 (lambda (x)
               ((igualar (comprobar (longitud x)))1) ))
...
(define concatenar
  (lambda (lst1)    ;Lista1
    (lambda(lst2)   ;Lista2
      (((Y (lambda (f)  ;Operador de punto fijo sobre la funcion f de argumento x
             (lambda (x)
               (lambda (y)
                 (((vacia? x)
                   (lambda (no_use)
                   ...
                   (lambda (no_use)
                     ...
                     ((construir (cabeza x)) ((f (cola x)) y)) ; re construye la lista 1, y cuando este vacia, adhiere la 
                     )
                   )
                  zero)  ; Pasa zero como argumento de no_use
                 ))
             ))  
        lst1) ; Pasa lst1 como el valor inicial de x.
       lst2) ; Pasa lst2 como el valor inicial de y.
      ))
  )
  ...
  (lambda (l)
    ((Y (lambda (f)
          (lambda (x)
            (((vacia? x)
              (lambda (no_use)
                zero         ;Si la lista es vacia devuelve 0
                )
              (lambda (no_use)
                ((sumnat un) (f (cola x)) ) ;Suma uno por cada vez que haya una cola
                )
              )
             zero)  ; Pasa zero como argumento de no_use
            )
...
;;; Elemento pertenece a lista

(define pertenece
  (lambda (lst)    ;Lista
    (lambda(elem)   ;elemento
      (((Y (lambda (f)  ;Operadoe de punto fijo sobre la funcion f de argumento x
             (lambda (x)
               (lambda (y)
                 (((vacia? x)
                   (lambda (no_use)
                     false    ;Si la lista es vacia devuelve falso porque el elemento no esta en una lista vacia
                     )
                   (lambda (no_use)
                     ( ((esigualent elem) (cabeza x));;Condicion que combrueba si la cabeza es igual al elemento buscado
                       true                          ;; Devuelve verdadero, si  hallo al eleemnto
                       ((f (cola x)) elem) )         ;; Si no lo halla, sigue buscando
                     )
                   )
                  zero)  ; Pasa zero como argumento de no_use
                 ))
             ))
        lst) ; Pasa lst como el valor inicial de x.
       elem) ; Pasa elem como el valor inicial de y.
      ))
  )
;;; Inversion
(define invertir
  (lambda (l)
    ((Y (lambda (f)
          (lambda (x)
            (( (vacia? x)
               (lambda (no_use)
                 vacia                                     ;Si la lista es vacia devuelve una lista vacia
                 )
               (lambda (no_use)
                 (     (concatenar (f (cola x)) )          ;;Concatena la lista en orden inverso
                       (  (construir (cabeza x)) vacia) )
                 )
               )
             vacia)  ; Pasa zero como argumento de no_use
            )
          ))
     l) ; Pasa l como el valor inicial de x.
    )
    ...
    ((Y (lambda (f)
          (lambda (x)
            (((vacia? x)
              ...
              (lambda (no_use)
                ((sument (cabeza x)) (f (cola x)) ) ;;Suma la cabeza a la suma de los demas elementos de la lista
                )
              )
             zero)  ; Pasa zero como argumento de no_use
            )
...
(define mayor 
  (lambda(x)
    (lambda(y)
      (((esmayorent x) y) ;;Comapara dos elementos y devuelve el mayor de ambos
       x
       y)
      ))
  )
    ...
    ((Y (lambda (f)
          (lambda (x)
            (((vacia? x)
              ...
              (lambda (no_use)
                ((mayor (cabeza x)) (f (cola x)) ) ;;Busca recursivamente el mayor de una lista comparando la cabeza con el resto de la cola
                ;; Devuelve el elemento de mayor tamaño
                )
              )
...
(define menor
  (lambda(x)
    (lambda(y)
      (((esmenorent x) y) ;Comapara dos elementos y devuelve el menor de ambos
       x
       y)
      ))
  )
    ...
    ((Y (lambda (f)
          (lambda (x)
            (((vacia? x)
              ...
              (lambda (no_use)
                ((menor (cabeza x)) (f (cola x)) ) ;;Busca recursivamente el menor de una lista comparando la cabeza con el resto de la cola
                ;; Devuelve el elemento de menor tamaño
                )
              )
             zero)  ; Pasa zero como argumento de no_use
            )
             ...
             (lambda (x)
               (lambda (y)
                 (((vacia? x)
                   ...
                   (lambda (no_use)
                     ( ((esigualent elem) (cabeza x));;Condicion
                       (cola x)                      ;;Dado un elemento, si la cabeza es el elemento, devuelve la lista sin el elemento
                       (     (concatenar (  (construir (cabeza x)) vacia) )  ;Sino sigue construyendo la misma lista
                             ((f (cola x)) elem) )
                       )
                     )
                   )
  ...
  (lambda (l)
    ((Y (lambda (f)
          (lambda (x)
            (( (vacia? x)
               (lambda (no_use)
                 vacia                 ;Si es vacia devuelve una lista vacia
                 )
               (lambda (no_use)
                 (     (concatenar (  (construir (min_lista x)) vacia) ) ;Si no es vacia Ubica el minimo en primera posicion
                       (f ((pop x)(min_lista x)) ) )                     ; Y lo concatena con la lista ordenada sin el minimo de la lista
                 )
               )
             vacia)  ; Pasa zero como argumento de no_use
            )
...
;;; Sumar listas de mismo tamaño
(define sumar_listas
  (lambda (l1)
    (lambda (l2)
      (
       ((neg ((esigualnat (longitud l1)) (longitud l2))) ;;Funcion que comprueba si dos listas son de la misma longitud y si lo son, las suma
        (lambda (no_use)
          vacia                           ;Si no lo son devuelve una lista vacia
          )
        (lambda (no_use)
          ((sumar_listas_aux l1) l2)              ;Si nlo son , hace la suma
          ) 
        )
       zero)
      )
...
(define sumar_listas_aux
  (lambda (lst1)    ;Lista1
    (lambda(lst2)   ;Lista2
      (((Y (lambda (f)  ;Operador de punto fijo sobre la funcion f de argumento x e y
             (lambda (x)
               (lambda (y)
                 (((vacia? x)  ;Si x es vacia devuelve una lista vacia
                   (lambda (no_use)
                     vacia
                     )
                   (lambda (no_use)
                     ( ((esigualnat (longitud x)) (longitud y))                ;;Condicion si son de la misma longitud
                       ( (construir ((sument (cabeza x)) (cabeza y) )) ((f (cola x)) (cola y))) ;Suma las cabezas de las listas y las concatena
                       ; Con la cola sumada de ambas listas
                       ((construir uno)vacia)  ;Si no son de la misma longitud, devuelve una lista vacia
                       )
                     )
                   )
                  zero)  ; Pasa zero como argumento de no_use
